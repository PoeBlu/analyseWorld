...
def fileList(foldPath=None, included=['*traj.bag_df.pickle'], excluded=[]):
    ...
    #get the folderpath if nothing is given
    if foldPath is None:
        foldPath=easygui.diropenbox()

    #walk through the structure and get a list of names
    for root, dirnames, filenames in os.walk(foldPath):

        #within the list of items, include only those which match the extension
        for include in included:
            for filename in fnmatch.filter(filenames, include):
                #flag to keep
                toKeep = True

                #if filename matches exclude, do not keep
                for exclude in excluded:
                    if exclude in filename:
                        toKeep = False

                #append if filter matched
                if toKeep:
